...
function createFakeShapeStream() {
  ...
  return {
    ...
    // Test helpers
    _emit(messages: any[]) { subscribers.forEach(cb => cb(messages)) },
    _setLoading(v: boolean) { _isLoading = v },
    _setLastSyncedAt(v: number) { _lastSyncedAt = v },
    _setError(e: any) { _error = e },
  }
}
...
function createFakeShape(stream: ReturnType<typeof createFakeShapeStream>) {
  ...
  return {
    stream,
    get currentRows() { return _currentRows },
    get currentValue() { return new Map(_currentRows.map((r, i) => [String(i), r])) },
    get lastOffset() { return _lastOffset },
    get handle() { return _handle },
    get error() { return stream.error },
    ...
    unsubscribeAll: vi.fn(),
    // Test helpers
    _setRows(rows: any[]) { _currentRows = rows },
    _setOffset(o: string) { _lastOffset = o },
    _setHandle(h: string) { _handle = h },
...
describe('useShape', () => {
  ...
  it('returns loading state initially', () => {
    const wrapper = mount(defineComponent({
      setup() {
        const { isLoading, data } = useShape({ url: 'http://localhost:3000/v1/shape', params: { table: 'items' } })
  ...
  it('updates data when shape notifies', async () => {
    const wrapper = mount(defineComponent({
      setup() {
        const { data, isLoading } = useShape({ url: 'http://localhost:3000/v1/shape', params: { table: 'items' } })
  ...
  it('cleans up subscription on unmount', () => {
    const wrapper = mount(defineComponent({
      setup() {
        const result = useShape({ url: 'http://localhost:3000/v1/shape', params: { table: 'items' } })
        return result
      },
      render() { return h('div') },
    }))
  ...
  it('handles error state', async () => {
    const wrapper = mount(defineComponent({
      setup() {
        const { error, isError } = useShape({ url: 'http://localhost:3000/v1/shape', params: { table: 'items' } })
