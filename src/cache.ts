...
export function sortedOptionsHash<T extends Row = Row>(options: ShapeStreamOptions<T>): string {
...
export function getShapeStream<T extends Row = Row>(
  ...
  const { signal: _signal, ...hashableOptions } = options as any;
  const hash = sortedOptionsHash(hashableOptions);
...
export function getShape<T extends Row = Row>(shapeStream: ShapeStream<T>): Shape<T> {
